...
| [arista-ceos-sandbox-agent/](arista-ceos-sandbox-agent/) | **[README.md](arista-ceos-sandbox-agent/README.md)** | 3-node Arista cEOS Containerlab fabric (`spine1` / `leaf1` / `leaf2`, eBGP underlay) **live** on Viper (`172.16.10.135`, 2026-08-17 evening ET). Local image **`ceos:4.33.9M`** from official `cEOS64-lab-4.33.9M.tar.xz` (`docker import` only — no Hub pull). containerlab **0.78.2**; BGP **Established**. kagent `SandboxAgent/arista-ceos` is **Ready**. Live CLI + kagent shots on the [README](arista-ceos-sandbox-agent/README.md); inspect/BGP in [REPORT.md](arista-ceos-sandbox-agent/REPORT.md). |
