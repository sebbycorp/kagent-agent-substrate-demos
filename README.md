...
| [arista-ceos-sandbox-agent/](arista-ceos-sandbox-agent/) | **[README.md](arista-ceos-sandbox-agent/README.md)** | 3-node Arista cEOS Containerlab fabric (`spine1` / `leaf1` / `leaf2`, eBGP underlay) on Viper for a later gVisor `SandboxAgent`. Image `sebbycorp/ceosimage` (not in git; Arista-licensed). **Build/deploy pending** — topology and verify scripts are in the folder; live screenshots and REPORT.md numbers stay empty until the lab is run on Viper. |
